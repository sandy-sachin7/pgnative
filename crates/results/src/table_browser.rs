...
pub fn build_sql_with_schema(
    ...
    if editability == pgnative_schema::model::types::Editability::Disabled {
...
#[must_use]
pub fn next_cursor(relation: &Relation, row: &crate::value::Row) -> Option<Vec<String>> {
    let cols = relation
        .primary_key
        .as_ref()
        .map(|pk| &pk.columns)
...
mod tests {
    ...
    use pgnative_schema::model::types::{ColumnId, Id, Oid, RelationKind};
    use pgnative_schema::model::types::{Nullability, TypeId, ValueSource};
