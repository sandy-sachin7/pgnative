...
use pgnative_schema::model::relation::Relation;
use pgnative_schema::model::types::Editability;
use thiserror::Error;
...
/// Callers map `pgnative_db::connection::TxState` → `TxForEdit` via `tx.to_string()`.
#[must_use]
    ...
    for ((k1, v1), (k2, v2)) in original.iter().zip(edited.iter()) {
        ...
        if v1 != v2 {
            ...
            if let Some(col) = rel.column_named(k1) {
                if col.value_source != pgnative_schema::model::types::ValueSource::Stored {
                    continue;
                }
            }
            out.push(ColumnDiff {
...
mod tests {
    ...
    use pgnative_schema::model::types::{Id, Nullability, Oid, RelationKind, TypeId, ValueSource};
