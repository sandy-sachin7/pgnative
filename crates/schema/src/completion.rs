...
use crate::model::types::{ColumnId, RelationId};
...
pub fn extract_aliases(sql: &str, cursor: usize) -> HashMap<String, RelationId> {
    ...
    for (alias, table) in raw {
        ...
        let id = crate::model::types::Id(hash);
        out.insert(alias, id);
...
mod tests {
    ...
    use crate::model::types::{Id, Oid, RelationKind};
    ...
    fn completion_alias_dot_suggests_columns() {
        ...
        use crate::model::types::{Nullability, ValueSource};
