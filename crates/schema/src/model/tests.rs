...
use crate::model::relation::{Function, PrimaryKey, Relation};
use crate::model::schema::Schema;
use crate::model::types::{
    Editability, Id, Nullability, Oid, RelationKind, SchemaId, TypeId, ValueSource,
};

fn sample_model() -> crate::model::index::SchemaModel {
    ...
    b.set_primary_key(
        rel,
        PrimaryKey {
            columns: vec![crate::model::types::Id(0)],
...
fn column_owner_resolves() {
    let m = sample_model();
    assert_eq!(m.owner_of(crate::model::types::Id(0)), Some(Id(0)));
}
